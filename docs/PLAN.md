...
| `scripts/view/art/base_art.gd` | Bases drawn in two passes: static architecture (masonry courses, planks, roofs, slits) rendered once per age and team into a cached SubViewport texture; banners, fire, smoke and night-lit windows drawn live. One base per age with turret mounts, damage states (cracks, fire), rebuild-on-evolve; turrets per kind and age with aim and recoil |
...
| `scripts/view/art/front_layer.gd` | In front of the lane, split at the seam like the backdrop: per-age weather (dust motes, gulls, drizzle, storm rain with splashes, ash and rising embers, neon rain) as stateless time-driven particles, plus foreground props on a faster parallax for depth |
...
| `scripts/view/art/day_night.gd` | Purely visual day/night cycle on match time (4-minute day, opens mid-morning): sky gradient, sun arc then moon and stars, warm twilight and cool night ambient, lamps and flash lights stronger at night. Scaled per age (strong in the daylit ages, gentle in Gunpowder and Future) so each age keeps its GDD §4.1 look. The sim never reads it; toggle in Settings |
