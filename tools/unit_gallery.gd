...
func _draw_panel(n: Node2D, mode: int) -> void:
	...
	var r: StringName = RaceLook.IDS[mode] if race == &"all" else race
	var rd := gd.race(r)
	var title: String = rd.display_name if race == &"all" else "%s — %s" % [["Colour", "Greyscale", "Silhouette"][mode], rd.display_name]
	n.draw_string(f, Vector2(10, 28), title, HORIZONTAL_ALIGNMENT_LEFT, -1, 22, Color(0.1, 0.1, 0.1))
	for c in ROLES.size():
